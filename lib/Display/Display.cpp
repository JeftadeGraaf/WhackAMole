...
void Display::drawHighscores()
{
    ...
    for (uint8_t i = 0; i < 5; i++)
    {
        ...
        _tft.print(123);
        _tft.setCursor(90, highscoresY);
        _tft.print("opponent");

        _tft.setCursor(164, highscoresY);
        _tft.print(123);
        _tft.setCursor(230, highscoresY);
        _tft.print("opponent");
        highscoresY += 18;
    }
}
...
{
    uint16_t xRounded = ((x - 2) / 10) * 10; // Round down to the nearest 10
    uint16_t yRounded = ((y - 2) / 10) * 10; // Round down to the nearest 10

    uint16_t xRounded2 = ((x - 2 + width + 4 + 9) / 10) * 10;  // Round up to the nearest 10
    uint16_t yRounded2 = ((y - 2 + height + 4 + 9) / 10) * 10; // Round up to the nearest 10
